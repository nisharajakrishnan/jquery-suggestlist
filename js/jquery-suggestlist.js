( function( $ ) {
    ...
    function Suggestlist( element, options ) {
        ...
        if ( this.isInput ) {
            this.element.on( {
                'focus.suggestlist': $.proxy( this.updateLi, this ),
    ...
    Suggestlist.prototype = {
        ...
        updateLi: function( event ) {
            if ( event ) {
                ...
                if ( event.ctrlKey || ! /^[0-9a-z ]$/.test( keyVal )&&(event.keyCode!=8 && this.picker.is( ':visible' ))) {
                    return;
                }
            }

            var val = /([^(min|hr)]*(min|hr)?)/.exec(this.element.val())[1],
